...
- (void)receiveData {
    ...
    [_networkManager sendGetRequest:request completionHandler:^(NSURLSessionDataTask *task, id responseObject, NSError *error) {
        ...
        [currentPage.homeListTableView.qc_header endRefreshingAnimation:NO completion:^{
            _backgroundView.scrollEnabled = YES;
        }];
        
        currentPage.homeListTableView.qc_insetTop = 0;
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if (collectionView == _backgroundView) {
        ...
        vc.homeListTableView.qc_header = [QCPullRefreshAnimationHeader headerWithRefreshingBlock:^{
            [self receiveData];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_currentIndex.row == indexPath.row || list.homeListTableView.qc_header.isRefreshing || _backgroundView == collectionView) {
        return;
    }
